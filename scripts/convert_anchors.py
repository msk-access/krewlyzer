...
def convert_tss_canonical(input_tsv: Path) -> pd.DataFrame:
    ...
    df_canonical["name"] = (
        "TSS|"
        + df_canonical[gene_name_col].fillna(df_canonical[gene_id_col]).astype(str)
        + "|"
        + df_canonical[transcript_id_col].astype(str)
    )
    ...
    # Build BED6
    bed = df_canonical[
        ["chrom", "bed_start", "bed_end", "name", "score", "bed_strand"]
    ].copy()
...
def merge_and_save(tss_bed: pd.DataFrame, ctcf_bed: pd.DataFrame, output_bed: Path):
    ...
    def chrom_sort_key(c):
        if c == "X":
            ...
            try:
                ...
                # A non-numeric, non-XY contig sorts last. Bare `except` here
                # also swallowed KeyboardInterrupt; unrelated to this change,
                # but the pre-commit hook lints scripts/ and CI does not, so it
                # surfaced when this directory gained a file.
                return 99
    ...
    logger.info(
        f"Merged anchors: {len(merged)} total ({tss_count} TSS + {ctcf_count} CTCF)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Ensembl BioMart files to ML-optimized WPS anchors (BED6)"
    )
    parser.add_argument(
        "--tss", type=Path, required=True, help="TSS TSV file from BioMart"
    )
    parser.add_argument(
        "--ctcf", type=Path, required=True, help="Regulatory features TSV from BioMart"
    )
