...
def filter_alu(input_bed: Path, output_bed: Path) -> tuple[int, int, Counter]:
    ...
    with opener(input_bed, "rt") as f_in, open(output_bed, "w") as f_out:
        for line in f_in:
            ...
            f_out.write(
                f"{chrom_norm}\t{start}\t{end}\t{rep_name}\t{score}\t{strand}\t{subfamily}\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter RepeatMasker for full-length Alu elements"
    )
    parser.add_argument(
        "--input", type=Path, required=True, help="Input RepeatMasker BED(.gz)"
    )
    parser.add_argument(
        "--output", type=Path, required=True, help="Output BED7 file with subfamily"
    )
    ...
    logger.info(
        f"Full-length Alu (280-320bp): {filtered:,} ({100*filtered/total:.1f}%)"
    )
