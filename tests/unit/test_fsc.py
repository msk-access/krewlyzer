...
def test_fsc_count_fragments_by_bins(tmp_path):
    ...
    ultra_shorts, core_shorts, mono_nucls, di_nucls, longs, totals, gcs = (
        _core.count_fragments_by_bins(bedgz, str(bins_file))
    )
...
def test_aggregate_by_gene_gene_mode(tmp_path):
    ...
    genes = {
        ...
        "Gene2": [
            SimpleNamespace(chrom="chr2", start=400, end=800, name="Gene2_target_01")
        ],
    }
