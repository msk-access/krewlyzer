...
def synthetic_gc_data() -> List[Dict]:
    ...
    for sample_idx in range(10):
        ...
        intermediate = [1.0 - 0.1 * np.cos(g * 8) + np.random.normal(0, 0.05) for g in gc]
...
def python_log_ratio(sample_val: float, expected_val: float, pseudocount: float = 1.0) -> float:
        ...
        for _, row in synthetic_fsd_data.iterrows():
            ...
            for col in synthetic_fsd_data.columns:
                if "-" in col:
                    ...
                    assert np.isfinite(log_ratio), f"Non-finite log-ratio for {arm}/{col}"
...
class TestWpsEquivalence:
    ...
    def test_wps_zscore_computation(self, synthetic_wps_data):
        ...
        for _, row in synthetic_wps_data.iterrows():
            ...
            assert np.isfinite(z_score), f"Non-finite z-score"
