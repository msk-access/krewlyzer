...
### Added
- **`docs/reference/example-output-report.md`** — a worked example of what
  `describe-output` produces, generated by `scripts/build_example_report.py`
  and checked in CI.

  Built from **fabricated** tables, deliberately. A real sample's report cannot
  live on a public docs site: identifier columns are redacted, but every
  fragment count and motif frequency in it is still that patient's data.
  Redaction removes the name, not the measurements. The fixture uses the same
  synthetic tables the contract-gate tests run against, so the example goes
  through the real rendering path rather than being a mock-up — and the
  generator refuses to write a file containing an identifier-shaped string.
